...
Transactions and Allocations default to the last 30 days, with All dates and Custom dates controls. Their data is no longer limited to the dashboard's eight recent transactions. Tables support text search, multi-select facets, signed amount ranges, sorting on every column (including mobile), filtered totals, and 25-row pagination. Categories show current rolling balances with overspent/empty/target status filters and explicit Edit and View transactions actions. A bulk reconciliation flow accepts desired current balances and records only the allocation deltas needed to reach them. Filters survive navigation between destinations within the current session. Review retains single-item and Resolve all controls and now handles failures visibly. Account creation, metadata editing, safe archive/removal, reconciliation, private Notion import, and sign-out are reachable from Accounts & settings.
...
The Notion importer distinguishes the payment-side `Partial Payment Of` relation from the numeric `Partial Payment` rollup on an expense. A versioned one-time repair reclassifies rows affected by the original truthy-`"0"` bug from card payments to expenses without changing their amounts, dates, accounts, category links, or ledger effect. This restores category spending and available balances while resolving only the review items created by that faulty classification.

Partial card-payment application, auto-allocation, obligation editing, passkeys, production institution approval, and scheduled notifications still need implementation/verification beyond this UI pass. The first bank-connection slice now includes encrypted provider connections, Plaid Link/exchange routes, account mapping, a deterministic demo provider, idempotent transaction sync, pending-to-posted updates, and review items. The current card-payment form only records a single-account outflow; it does not yet match expenses or update a receiving card. These remaining items are requirements, not completed capabilities.

...
export default function LoginPage() {
  ...
  useEffect(() => {
    fetch("/api/auth/bootstrap").then((response) => response.json()).then((body) => {
      setBootstrapAvailable(Boolean(body.available));
      setBootstrapError(typeof body.error === "string" ? body.error : "");
      if (body.available) setMode("create");
    }).catch(() => setBootstrapError("Unable to check first-time setup. Please refresh after the database is ready."));
  }, []);

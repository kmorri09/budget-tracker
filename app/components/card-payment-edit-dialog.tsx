  ...
  return <dialog ref={dialog} className="entry-dialog coverage-dialog" aria-labelledby={titleId} onCancel={event => { if (busy) event.preventDefault(); else onClose(); }}>
    ...
    <form onSubmit={async event => {
      ...
      <fieldset disabled={busy}>
        ...
        <div className="form-grid"><label>From account<select name="fromAccountId" defaultValue={payment.fromAccountId} required>{fromAccounts.map(account => <option key={account.id} value={account.id}>{account.name}{account.active ? "" : " (inactive)"}</option>)}</select></label><label>To credit card<select name="toAccountId" value={toAccountId} onChange={event => { setToAccountId(event.target.value); setApplications({}); }} required>{toAccounts.map(account => <option key={account.id} value={account.id}>{account.name}{account.active ? "" : " (inactive)"}</option>)}</select></label></div>

...
export default function CardCoverageReconcileDialog({ dashboard, onClose, onSaved }: { dashboard: DashboardData; onClose: () => void; onSaved: (changed: number, state: "paid" | "unpaid") => void }) {
  ...
  return <dialog ref={dialog} className="entry-dialog coverage-dialog" aria-labelledby={titleId} onCancel={event => { if (busy) event.preventDefault(); else onClose(); }}>
    ...
    <fieldset disabled={busy}>
      <div className="coverage-tools"><label>Find purchases<input type="search" placeholder="Description or category…" value={search} onChange={event => setSearch(event.target.value)} /></label><label>Credit card<select value={card} onChange={event => setCard(event.target.value)}><option>All cards</option>{cards.map(name => <option key={name}>{name}</option>)}</select></label><label>Current coverage<select value={coverage} onChange={event => setCoverage(event.target.value)}><option>All statuses</option><option>Unpaid</option><option>Partially paid</option><option>Paid</option></select></label></div>

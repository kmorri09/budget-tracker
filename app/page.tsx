...
export default function Home() {
  ...
  useEffect(() => {
    ...
    document.querySelector<HTMLAnchorElement>(".mobile-more-menu a")?.focus();
  ...
  function followNavigation(event: MouseEvent<HTMLAnchorElement>, next: Destination) {
  ...
  return <main className="app-shell workspace">
    ...
    <aside className="sidebar">
      ...
      <nav className="side-nav" aria-label="Primary navigation">{navigation.map(item => <a key={item.key} href={`/#${item.key}`} className={"nav-item " + (destination === item.key ? "active" : "")} aria-current={destination === item.key ? "page" : undefined} onClick={event => followNavigation(event, item.key)}><span aria-hidden="true"><NavigationIcon item={item} /></span>{item.label}{item.key === "review" && !!dashboard?.reviews.length && <em>{dashboard.reviews.length}</em>}</a>)}</nav>
    ...
    <div className="content-wrap" id="workspace-content" tabIndex={-1}>
      ...
      {dashboard && <>
        ...
        <section hidden={destination !== "obligations"} aria-label="Obligations">
          <div className="view-heading"><div><p>Through {planningThrough}: <strong>{planningWindow.length} upcoming · {money(planningWindow.reduce((sum, item) => sum + item.expectedAmount, 0))} expected</strong>. Upcoming charges appear first; dates and amounts update from matched withdrawals.</p><label className="planning-through">Plan through <input type="date" value={planningThrough} min={today()} onChange={event => setPlanningThrough(event.target.value || dateInDays(14))} /></label></div><div className="section-actions"><button className="secondary-button" onClick={() => setFundingObligations(true)}>Fund upcoming</button><button className="primary-button" onClick={() => setObligationEditor("new")}>＋ Add obligation</button></div></div>
          <DataTable title="Obligations" amountKey="expectedAmount" amountLabel="Expected total" initialSort="sortDate" initialDirection="asc" rows={dashboard.obligations.map(obligation => ({ id: obligation.id, name: obligation.name, sortDate: obligation.active && !obligation.covered ? obligation.nextChargeDate : "9999-12-31", nextChargeDate: obligation.covered ? "—" : obligation.nextChargeDate, expectedAmount: obligation.expectedAmount, amountBasis: obligation.amountSource === "observed" ? "Latest charge" : "Planned", change: obligation.lastCharge ? `${obligation.previousCharge ? "vs prior" : "vs plan"} ${money(obligation.lastCharge.amount - (obligation.previousCharge?.amount ?? obligation.amount))}` : "No history", lastCharge: obligation.lastCharge ? `${obligation.lastCharge.date} · ${money(obligation.lastCharge.amount)}` : "None yet", category: obligation.category, account: obligation.account, cadence: obligation.cadence || "One time", status: !obligation.active ? "Inactive" : obligation.covered ? "Paid" : obligation.nextChargeDate < today() ? "Overdue" : "Upcoming" }))}
            columns={[{ key: "name", label: "Name" }, { key: "nextChargeDate", label: "Next charge" }, { key: "expectedAmount", label: "Expected", money: true }, { key: "amountBasis", label: "Based on" }, { key: "change", label: "Change" }, { key: "lastCharge", label: "Last charge", detail: true }, { key: "category", label: "Category" }, { key: "status", label: "Status" }, { key: "account", label: "Account", detail: true }, { key: "cadence", label: "Cadence", detail: true }]}
            ...
            rowActions={[{ label: "Confirm suggested charge", isEligible: row => Boolean(dashboard.obligations.find(item => item.id === row.id)?.suggestion), onClick: row => { const obligation = dashboard.obligations.find(item => item.id === row.id); if (obligation?.suggestion) void decideObligationMatch(obligation, obligation.suggestion, "confirmed"); } }, { label: "Dismiss suggestion", isEligible: row => Boolean(dashboard.obligations.find(item => item.id === row.id)?.suggestion), onClick: row => { const obligation = dashboard.obligations.find(item => item.id === row.id); if (obligation?.suggestion) void decideObligationMatch(obligation, obligation.suggestion, "dismissed"); } }, { label: "Unmatch last charge", isEligible: row => Boolean(dashboard.obligations.find(item => item.id === row.id)?.lastCharge), onClick: row => { const obligation = dashboard.obligations.find(item => item.id === row.id); if (obligation?.lastCharge) void decideObligationMatch(obligation, obligation.lastCharge, "dismissed"); } }, { label: "Edit", onClick: row => { const obligation = dashboard.obligations.find(item => item.id === row.id); if (obligation) setObligationEditor(obligation); } }, { label: "Deactivate", isEligible: row => row.status !== "Inactive", onClick: row => { const obligation = dashboard.obligations.find(item => item.id === row.id); if (obligation) void updateObligationStatus(obligation, false); } }, { label: "Reactivate", isEligible: row => row.status === "Inactive", onClick: row => { const obligation = dashboard.obligations.find(item => item.id === row.id); if (obligation) void updateObligationStatus(obligation, true); } }]} />
          {dashboard.obligations.filter(item => item.suggestion).map(item => <p className="obligation-suggestion" key={item.id}><strong>{item.name}:</strong> Possible withdrawal {item.suggestion!.description} · {item.suggestion!.date} · {money(item.suggestion!.amount)}. Confirm or dismiss it in the table above.</p>)}
        </section>
        <section hidden={destination !== "review"} aria-label="Review"><ReviewInbox dashboard={dashboard} onEdit={editTransaction} onRecordPayment={recordImportedPayment} onChanged={() => { setToast("Review updated"); void refresh(); }} /></section>
  <section hidden={destination !== "accounts"} aria-label="Accounts"><Accounts dashboard={dashboard} onAction={openAction} onChanged={(message) => { setToast(message ?? "Account updated"); void refresh(); }} /></section>
    ...
    {editingTransaction && dashboard && <TransactionEditDialog transaction={editingTransaction.transaction} categorySuggestion={editingTransaction.suggestion} dashboard={dashboard} onClose={() => setEditingTransaction(null)} onSaved={(message) => { setEditingTransaction(null); setToast(message); void refresh(); }} />}
  ...
  async function decideObligationMatch(obligation: DashboardData["obligations"][number], candidate: { id: string; type: "transaction" | "payment" }, status: "confirmed" | "dismissed") {
    setError("");
    try {
      const response = await fetch("/api/obligations/matches", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ obligationId: obligation.id, candidateType: candidate.type, candidateId: candidate.id, status }) });
      const result = await response.json().catch(() => null);
      if (!response.ok) throw new Error(result?.error ?? "Could not save match decision.");
      setToast(status === "confirmed" ? "Charge matched to obligation" : "Charge match dismissed"); void refresh();
    } catch (failure) { setError(failure instanceof Error ? failure.message : "Could not save match decision."); }
  }
...
function Overview({ dashboard: data, navigate, onAction, onReconcileBudget, onFundObligations, viewCategory }: { dashboard: DashboardData; navigate: (destination: Destination, categoryStatus?: string) => void; onAction: (action: ActionType) => void; onReconcileBudget: () => void; onFundObligations: () => void; viewCategory: (category: string) => void }) {
  ...
  return <>
    ...
    <div className="main-grid"><article className="panel"><div className="section-heading"><div><h2>Category balances</h2><p className="section-context">Allocated in the last 28 days</p></div><a className="text-link" href="/#categories" onClick={event => followPage(event, "categories")}>All categories →</a></div>{!data.categories.length && <p className="empty-state">Add categories to start planning your money.</p>}{recentlyAllocated.map(category => <div className="overview-row" key={category.id}><button className="text-link" onClick={() => viewCategory(category.name)}>{category.name}</button><span className={category.available < 0 ? "negative" : ""}>{money(category.available)}<small>{category.available < 0 ? "Overspent" : "Available"}</small></span></div>)}{!!data.categories.length && !recentlyAllocated.length && <p className="empty-state">No categories were allocated money in the last 28 days.</p>}</article>
      <div className="right-stack"><article className="panel"><div className="section-heading"><h2>Needs attention</h2></div><a className="attention-link" href="/#review" onClick={event => followPage(event, "review")}><span>Review inbox</span><strong>{data.reviews.length} open →</strong></a><button className="attention-link" onClick={() => navigate("categories", "Overspent")}><span>Overspent categories</span><strong>{overspent.length} →</strong></button></article><article className="panel"><div className="section-heading"><h2>Upcoming obligations</h2>{!!activeObligations.length && <button className="text-link" onClick={onFundObligations}>Fund upcoming →</button>}</div>{activeObligations.length ? [...activeObligations].sort((a, b) => a.nextChargeDate.localeCompare(b.nextChargeDate)).map(item => <div className="overview-row" key={item.id}><span>{item.name}<small>{item.nextChargeDate} · {item.category}</small></span><strong>{money(item.expectedAmount)}</strong></div>) : <p className="empty-state">No active obligations yet.</p>}<a className="text-link" href="/#obligations" onClick={event => followPage(event, "obligations")}>Manage obligations →</a></article></div></div>
    <article className="panel activity-panel"><div className="section-heading"><h2>Recent transactions</h2><a className="text-link" href="/#transactions" onClick={event => followPage(event, "transactions")}>All transactions →</a></div>{data.activity.slice(0, 8).map(entry => <div className="overview-row" key={entry.id}><span>{entry.description}<small>{entry.date} · {entry.account} · {entry.category ?? kindLabel(entry.kind)}</small></span><strong className={signedAmount(entry) < 0 ? "negative" : ""}>{money(signedAmount(entry))}</strong></div>)}{!data.activity.length && <p className="empty-state">No transactions yet. Add one or import your Notion snapshot from Accounts.</p>}</article>
  </>;
}

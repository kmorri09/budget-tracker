...
function signedMoney(kind: string, value: number) {
  const positive = kind === "income" || kind === "refund" || (kind === "adjustment" && value >= 0);
  return `${positive ? "+" : "-"}${money(Math.abs(value))}`;
}
...
type DashboardData = {
  ...
  accounts: { id: string; name: string; institution: string; type: string; syncEnabled: boolean; openingBalance: number; providerBalance: number | null; providerBalanceAt: string | null; ledgerBalance: number }[];
        ...
        {activeNav === "home" ? (
          <>
            ...
            <section className="main-grid">
              ...
              <div className="right-stack"><div className="panel review-panel"><div className="section-heading"><div><p className="eyebrow">Needs your attention</p><h2>Review inbox <span className="count-badge">{dashboard?.reviews.length ?? 0}</span></h2></div><button className="link-button" onClick={() => setActiveNav("review")}>Open <span>→</span></button></div>{(dashboard?.reviews.length ?? 0) === 0 ? <div className="review-item"><span className="review-mark">✓</span><div><strong>No items need review</strong><p>Imported activity will appear here when connected.</p></div></div> : <div className="review-item"><span className="review-mark">!</span><div><strong>{dashboard?.reviews.length ?? 0} items need review</strong><p>Open the Review inbox to resolve imported activity.</p></div></div>}</div><div className="panel upcoming-panel"><div className="section-heading"><div><p className="eyebrow">Coming up</p><h2>Upcoming obligations</h2></div><button className="icon-button small" onClick={() => setActiveNav("budget")} aria-label="View all obligations">→</button></div><div className="upcoming-row"><span className="date-tile"><b>—</b><small>DATE</small></span><div><strong>No obligations yet</strong><p>Add an obligation when you are ready.</p></div><span className="amount">$0.00</span></div></div></div>
            </section>
...
function ReviewInbox({ dashboard }: { dashboard: DashboardData }) {
  const [busy, setBusy] = useState(false);
  async function resolveAll() {
    if (!window.confirm(`Resolve all ${dashboard.reviews.length} open review items? This does not change the underlying transactions.`)) return;
    setBusy(true);
    await fetch("/api/reviews", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ all: true, status: "resolved" }) });
    window.location.reload();
  }
  return <section className="release-page"><div className="section-heading"><div><p className="eyebrow">Inbox</p><h2>Review before it changes your plan</h2></div>{dashboard.reviews.length > 0 && <button className="secondary-button" type="button" disabled={busy} onClick={() => { void resolveAll(); }}>{busy ? "Resolving…" : `Resolve all (${dashboard.reviews.length})`}</button>}</div><div className="panel release-list">{dashboard.reviews.length === 0 ? <p className="empty-state">Nothing needs review right now.</p> : dashboard.reviews.map((review) => <div className="review-item" key={review.id}><span className="review-mark">!</span><div><strong>{review.title}</strong><p>{review.details}</p></div><button className="secondary-button" onClick={async () => { await fetch("/api/reviews", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: review.id, status: "resolved" }) }); window.location.reload(); }}>Resolve</button></div>)}</div></section>;
}

function AccountConfiguration({ dashboard }: { dashboard: DashboardData }) {
  const [reconcilingId, setReconcilingId] = useState<string | null>(null);
  const [drafts, setDrafts] = useState<Record<string, string>>({});
  const [error, setError] = useState("");
  return <section className="release-page"><div className="section-heading"><div><p className="eyebrow">Workspace</p><h2>Accounts & configuration</h2></div><div className="section-actions"><a className="secondary-button" href="/import">↥ Import Notion</a></div></div><div className="panel release-list"><p className="auth-copy">Reconcile against today&apos;s provider balance without changing or reclassifying any historical transaction.</p>{error && <p className="form-error" role="alert">{error}</p>}{dashboard.accounts.map((account) => { const value = drafts[account.id] ?? (account.providerBalance === null ? "" : String(Math.abs(account.providerBalance))); const entered = Number(value); const target = account.type === "credit_card" ? -Math.abs(entered) : entered; const delta = value === "" || !Number.isFinite(entered) ? null : target - account.ledgerBalance; return <div className="account-row" key={account.id}><div><strong>{account.name}</strong><p>{account.institution} · {account.type}{account.syncEnabled ? " · sync enabled" : " · manual"}</p>{reconcilingId === account.id && <form className="inline-form account-balance-form" onSubmit={async (event) => { event.preventDefault(); setError(""); const response = await fetch("/api/accounts", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: account.id, providerBalance: new FormData(event.currentTarget).get("providerBalance") }) }); const result = await response.json().catch(() => null) as { error?: string } | null; if (!response.ok) { setError(result?.error ?? "Could not reconcile account"); return; } window.location.reload(); }}><label>Today&apos;s provider balance<input name="providerBalance" type="number" step="0.01" placeholder={account.type === "credit_card" ? "Amount owed" : "0.00"} value={value} onChange={(event) => setDrafts((current) => ({ ...current, [account.id]: event.target.value }))} required /></label>{delta !== null && <p className="reconciliation-delta">Force reconcile will {delta >= 0 ? "add" : "subtract"} <strong>{money(Math.abs(delta))}</strong> to this account.</p>}<div className="section-actions"><button className="primary-button" type="submit">Force reconcile</button><button className="secondary-button" type="button" onClick={() => setReconcilingId(null)}>Cancel</button></div></form>}</div><div className="account-actions"><span className="amount">{money(account.ledgerBalance)}</span>{account.providerBalance !== null && <small className="account-provider">Provider {money(Math.abs(account.providerBalance))}</small>}<button className="link-button" type="button" onClick={() => setReconcilingId(reconcilingId === account.id ? null : account.id)}>{reconcilingId === account.id ? "Close" : "Force reconcile"}</button></div></div>; })}</div></section>;
}

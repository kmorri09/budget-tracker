...
function Overview({ dashboard: data, navigate, onAction, onReconcileBudget, viewCategory }: { dashboard: DashboardData; navigate: (destination: Destination) => void; onAction: (action: ActionType) => void; onReconcileBudget: () => void; viewCategory: (category: string) => void }) {
  ...
  return <>
    ...
    <div className="main-grid"><article className="panel"><div className="section-heading"><div><h2>Category balances</h2><p className="section-context">Allocated in the last 28 days</p></div><button className="text-link" onClick={() => navigate("categories")}>All categories →</button></div>{!data.categories.length && <p className="empty-state">Add categories to start planning your money.</p>}{recentlyAllocated.slice(0, 6).map(category => <div className="overview-row" key={category.id}><button className="text-link" onClick={() => viewCategory(category.name)}>{category.name}</button><span className={category.available < 0 ? "negative" : ""}>{money(category.available)}<small>{category.available < 0 ? "Overspent" : "Available"}</small></span></div>)}{!!data.categories.length && !recentlyAllocated.length && <p className="empty-state">No categories were allocated money in the last 28 days.</p>}{recentlyAllocated.length > 6 && <button className="text-link recent-more" onClick={() => navigate("categories")}>View {recentlyAllocated.length - 6} more recently allocated {recentlyAllocated.length - 6 === 1 ? "category" : "categories"} →</button>}</article>
